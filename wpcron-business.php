...
function getplink($itemlink){
	...
	echo "SELECT link_id,link_pid FROM $plink_table WHERE link_url LIKE '%".addslashes($itemlink)."%'";
	echo "<br/>";
	echo $user_count = $wpdb->get_var( "SELECT link_id,link_pid FROM $plink_table WHERE link_url LIKE '%".addslashes($itemlink)."%'");
	echo "<br/>";
	return $user_count;
}
...
function insertplink($itemlink,$post_id){
	...
	$itemlink = str_replace("'","",$itemlink);
	echo "<br/>";
	echo $sql = "INSERT INTO ".$plink_table." (link_url, link_pid,category) VALUES ('".addslashes($itemlink)."', ".$post_id.",'business');";
	echo "<br/>";  
	$wpdb->query($sql);
}
...
$term = term_exists('Business', 'category');
if ($term == 0 && $term == null) {
  wp_insert_term( 'Business', 'category');
  $term = term_exists('Business', 'category');
}

$feed = 'http://www.abc.net.au/news/feed/51892/rss.xml';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
...
$feed = 'http://rss.nytimes.com/services/xml/rss/nyt/Business.xml';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
	if(count($Items_data ) > 0){
		foreach ($Items_data as $items) {
			...
			if(empty($user_count)) {
				...
				wp_set_post_terms( $post_id, 44, 'news-provider-category');
				$td_post_theme_settings = array();
				$td_post_theme_settings['td_source'] =  'New York Times';
				$td_post_theme_settings['td_source_url'] =  $feed;
				$td_post_theme_settings['td_via'] =  'New York Times';
				$td_post_theme_settings['td_via_url'] =  $items['link'];
				update_post_meta($post_id, 'td_post_theme_settings', $td_post_theme_settings, true);
				if(!empty($mediaurl)) {
					fetch_media($mediaurl,$post_id);
...
/*WSJ Online */
$feed = 'http://feeds.wsjonline.com/wsj/video/business/feed';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
...
$feed = 'https://www.wired.com/category/transportation/feed/';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
	...
	if(count($Items_data ) > 0){
		foreach ($Items_data as $items) {
			...
			if(empty($user_count)) {
				...
				wp_set_post_terms( $post_id, 74, 'news-provider-category');
				$td_post_theme_settings = array();
...
/*(updatedesc('www.abc.net.au/news/feed/51892/rss.xml');
updatedesc('feeds.reuters.com/reuters/businessNews');
updatedesc('rss.cnn.com/rss/money_latest.rss');
updatedesc('feeds.bbci.co.uk/news/business/rss.xml');
updatedesc('rss.nytimes.com/services/xml/rss/nyt/Business.xml');
updatedesc('www.wired.com/category/transportation/feed/');
updatedesc('feeds.wsjonline.com/wsj/video/business/feed');*/

echo "Business category success";
exit;

...
function insertplink($itemlink,$post_id){
	...
	$plink_table = $wpdb->prefix."plinks";
	$itemlink = str_replace("'","",$itemlink);
	$sql = "INSERT INTO ".$plink_table." (link_url, link_pid,category) VALUES ('".addslashes($itemlink)."', ".$post_id.",'technology');";    
	$wpdb->query($sql);
}
...
function updatedesc($feed){
	$feed = 'https://docs.wprssaggregator.com/fulltext/makefulltextfeed.php?url='.$feed.'&max=10&links=preserve&exc=&submit=Create+Feed';
	if(!empty($feed)) {
...
$feed = 'http://rss.cnn.com/rss/edition_technology.rss';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
...
$feed = 'http://rss.cnn.com/rss/money_technology.rss';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
...
$feed = 'http://feeds.foxnews.com/foxnews/tech';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
	if(count($Items_data ) > 0){
		foreach ($Items_data as $items) {
			...
			if(empty($user_count)) {
				...
				$post_id =  wp_insert_post( $my_post );
				if($items['media:group']['media:content']['5_attr']['url']){
					$mediaurl = $items['media:group']['media:content']['5_attr']['url'];
				}else if($items['media:group']['media:content']['4_attr']['url']){
					$mediaurl = $items['media:group']['media:content']['4_attr']['url'];
				}else if($items['media:group']['media:content']['3_attr']['url']){
					$mediaurl = $items['media:group']['media:content']['3_attr']['url'];
				}else if($items['media:group']['media:content']['2_attr']['url']){
					$mediaurl = $items['media:group']['media:content']['2_attr']['url'];
				}else if($items['media:group']['media:content']['1_attr']['url']){
					$mediaurl = $items['media:group']['media:content']['1_attr']['url'];
				} else {
					$mediaurl = '';
				}
...
$feed = 'http://feeds.bbci.co.uk/news/video_and_audio/technology/rss.xml';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
...
$feed = 'http://www.wsj.com/xml/rss/3_7455.xml';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
	/*echo "<pre>"; print_r($Items_data); exit;*/
	if(count($Items_data ) > 0){
		foreach ($Items_data as $items) {
			$description = strreplcecode($items['description']);
			$title = addslashes($items['title']);
			...
			if(empty($user_count)) {
				...
				$post_id =  wp_insert_post( $my_post );
				$mediaurl = '';
				if($items['media:content_attr']['height'] >= 300 && $items['media:content_attr']['width'] >= 300) {
					$mediaurl = $items['media:content_attr']['url'];
				}	
				wp_set_post_terms( $post_id, 62, 'news-provider-category');
				$td_post_theme_settings = array();
				$td_post_theme_settings['td_source'] =  'WSJ';
				$td_post_theme_settings['td_source_url'] =  $feed;
				$td_post_theme_settings['td_via'] =  'WSJ';
				$td_post_theme_settings['td_via_url'] =  $items['link'];
				update_post_meta($post_id, 'td_post_theme_settings', $td_post_theme_settings, true);
				if(!empty($mediaurl)) {
					fetch_media($mediaurl,$post_id);
...
$feed = 'https://www.ttgmedia.com/Syndication/DF.cfm?f=33&ft=10&preview=1';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
	...
	if(count($Items_data ) > 0){
		foreach ($Items_data as $items) {
			...
			if(empty($user_count)) {
				...
				wp_set_post_terms( $post_id, 64, 'news-provider-category');
				$td_post_theme_settings = array();
				$td_post_theme_settings['td_source'] =  'TTG Media';
				$td_post_theme_settings['td_source_url'] =  $feed;
				$td_post_theme_settings['td_via'] =  'TTG Media';
				$td_post_theme_settings['td_via_url'] =  $items['link'];
				update_post_meta($post_id, 'td_post_theme_settings', $td_post_theme_settings, true);
				if(!empty($mediaurl)) {
					fetch_media($mediaurl,$post_id);
...
$feed = 'http://www.news18.com/rss/tech.xml';
if(!empty($feed)) {
	$res=send_request('GET', $feed);
	$data = xml2array($res);
	$Items_data = convertdata($data,$type=$rss_url[0]);
	if(count($Items_data ) > 0){
		foreach ($Items_data as $items) {
			...
			if(empty($user_count)) {
				...
				wp_set_post_terms( $post_id, 75, 'news-provider-category');
				$td_post_theme_settings = array();
				$td_post_theme_settings['td_source'] =  'News18';
				$td_post_theme_settings['td_source_url'] =  $feed;
				$td_post_theme_settings['td_via'] =  'News18';
				$td_post_theme_settings['td_via_url'] =  $items['link'];
				update_post_meta($post_id, 'td_post_theme_settings', $td_post_theme_settings, true);
				if(!empty($mediaurl)) {
					fetch_media($mediaurl,$post_id);
...
/*only  Description*/
updatedesc('www.news18.com/rss/tech.xml');
updatedesc('feeds.reuters.com/reuters/technologyNews');
updatedesc('rss.cnn.com/rss/edition_technology.rss');
updatedesc('rss.cnn.com/rss/money_technology.rss');
updatedesc('feeds.foxnews.com/foxnews/tech');
updatedesc('feeds.bbci.co.uk/news/video_and_audio/technology/rss.xml');
updatedesc('www.wsj.com/xml/rss/3_7455.xml');
updatedesc('www.ttgmedia.com/Syndication/DF.cfm?f=33&ft=10&preview=1');

echo "Technology category success";
exit;
